...
public class Main {
    ...
    private static List<String> terSymbolList; // 终结符
    private static List<String> nonTerSymbolList; // 非终结符
    private static final HashMap<String, Set<String>> formulaSetMap = new HashMap<>(); //产生式
    private static String sentence; // 待分析句型
    private static HashMap<String, List<String>> firstvtSets;
    private static HashMap<String, List<String>> lastvtSets;
    ...
    public static void main(String[] args) {
        ...
        System.out.print(priorityTable);
        try {
            analyze();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static void analyze() throws Exception {
        ...
        do {
            // Print
            System.out.format("%-20s", S.stream().limit(k + 1).reduce((result, element) -> result + element).get());
        ...
        sentence = temp;
        System.out.format("Sentence \"%s\" is valid for the formula\n", sentence);;
    }

    private static void generatePriorityTable() {
    ...
    static private boolean isTerSymbol(char c) {
        return terSymbolList.contains(String.valueOf(c));
    }
    static private boolean isNonTerSymbol(char c) {
        return nonTerSymbolList.contains(String.valueOf(c));
    }
    private static void generateLastvtSets() {
    ...
    private static void initFirstSetsAndLastSets() {
    ...
    private static void generateFirstvtSets() {
        F = new boolean[nonTerSymbolList.size()][terSymbolList.size()];
        for (Map.Entry<String, Set<String>> entry : formulaSetMap.entrySet()) {
            String left = entry.getKey();
            Set<String> rights = entry.getValue();
    ...
    private static boolean getGridOfF(String P, String a) {
        return F[nonTerSymbolList.indexOf(P)][terSymbolList.indexOf(a)];
    }
    private static void setGridOfF(String P, String a, boolean value) {
        F[nonTerSymbolList.indexOf(P)][terSymbolList.indexOf(a)] = value;
    }

    private static void generateDataStructure(String content) {
